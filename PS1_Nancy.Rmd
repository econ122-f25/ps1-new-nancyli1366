...
Fewer cylinders are correlated with higher mpg in this filtered subset.
---
...
```{r}
# Your code here
mtcars_processed_1 <- mtcars_processed %>%
...
hp_per_wt allows easy comparison between cars of different sizes, while qsec_category makes results easier to interpret at a glance instead of analyzing the raw data. 
---
...
```{r}
# Your code here
mtcars_summary <- mtcars_processed %>%
  group_by(cyl, am) %>%
  summarize(
...
First of all, as cylinders increase, average mpg drops and median hp rises. Then, for cars with the same cylinders (4 and 6 cylinders), manuals tend to have slightly higher mpg than automatics, and automatics show higher median hp. 
---
...
product_sales_long <- product_sales_wide %>%
  ...
  mutate(
    Year = as.numeric(gsub("Year_", "", Year))
  )
product_sales_long
```
...
I think long format is more concise and scalable; it is preferred because we can avoid creating Year_ and Growth_ columns for every year, which is more easy to operate.
---
...
students_inner <- students_demographics %>%
  inner_join(student_grades, by = "student_id") 
#keeps only rows that match in both datasets 

students_left <- students_demographics %>%
  left_join(student_grades, by = "student_id") 
#keeps all rows from the demographic dataset and fills missing grades with NA 

students_ave <- students_left %>%
  group_by(student_id, student_name) %>%
  summarise(ave_grade = mean(grade_score, na.rm = TRUE))
#computes the mean grade per student
...
S005 and S007 are present in the inner_join but not in the left_join because they don't have the data for grades. For students with multiple grades, the calculation condenses them into a single average score, such as for Alice; for students without grades, the result is NaN. 
...
The semi_join() shows which students from the demographics table actually have grade records. If fewer students appear than expected, it can reveal missing or delayed grade data. The anti_join() on demographics highlights students who are in the roster but do not yet have any grades. This is useful for spotting gaps in coverage and making sure grade reporting is complete. The anti_join() on grades reveals orphan grade records—entries in the grade table that do not match any current student in the demographics table. They point directly to data quality and integrity issues. Together, these joins provide a clear picture of completeness. They separate students with valid records, those missing grades, and unmatched grade entries, making it easier to validate data and ensure consistency across student systems.
